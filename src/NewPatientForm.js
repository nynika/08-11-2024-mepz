...
const handleTempChange = (e) => {
    ...
    setFormData((prevState) => {
      const updatedFormData = {
        ...
        if (name === "dob") {
         ...
         // Calculate BMI field is updated
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {const response = await axios.post(`https://www.relainstitute.in/NewHIS/api/his/update_Mepz_tb`,formData);
...
<Form.Group controlId="dob">
  ...
  <div style={{ maxWidth: "20px", fontSize: "0.10rem" }}>
    <DayPicker
      mode="single"
      selected={dob}
